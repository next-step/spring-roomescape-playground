...
- 사용자는 예약 관리 페이지에서 예약을 추가할 수 있다.
  - 예약 시간은 직접 입력하지 않고, 등록된 시간 중에서 선택한다.
...
- [x] `GET /reservation` 요청 시 예약 관리 페이지(`new-reservation.html`)를 응답한다.
- [x] `GET /reservations` 요청 시 전체 예약 목록을 JSON으로 응답한다.
  - [x] 각 예약은 `id`, `name`, `date`, `time`(시간 객체) 필드를 가진다.
- [x] `POST /reservations` 요청 시 예약을 추가한다.
  - [x] 요청의 `time`은 시간의 식별자이며, 해당 시간을 찾아 예약을 생성한다.
...
- [x] 잘못된 요청은 적절한 Status Code로 응답한다.
  ...
  - [x] 예약 추가 시 `time`이 1 이상의 숫자가 아니거나(기존 `"10:00"` 방식 포함) 존재하지 않는 시간이면 `400 Bad Request`로 응답한다.
...
- [x] `schema.sql`에 예약(`reservation`) 테이블 생성 쿼리를 작성한다.
  - [x] `id`(PK), `name`, `reserved_date`, `time_id` 컬럼을 가진다.
  - [x] `time_id`는 외래키로 `time` 테이블의 `id`를 참조한다.
  - [x] 같은 `reserved_date`, `time_id`에 예약이 중복될 수 없도록 유니크 제약을 건다.
- [x] 예약 조회 시 `inner join`으로 시간 정보를 함께 조회한다.
- [x] h2 데이터베이스를 설정한다.
