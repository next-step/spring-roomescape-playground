package roomescape.time.presentation_layer;
...
public class TimeController {
    ...
    public ResponseEntity<List<TimeEntity>> getTimes() {
        // DB에서 예약 정보를 가져와서 반환
        List<TimeEntity> response = timeService.getTimes();
    ...
    // 예약 추가
    @PostMapping("/times")
    public  ResponseEntity<?> saveTime(@RequestBody TimeEntity time) {
        ...
        // 예약 추가
        TimeEntity savedTime = timeService.saveTime(time.getTime());
    ...
    public ResponseEntity<?> deleteTime(@PathVariable Long id) {
        ...
        TimeEntity existingTIme = timeService.getTime(id);
